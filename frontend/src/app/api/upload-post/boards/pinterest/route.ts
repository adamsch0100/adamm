...
/**
 * GET /api/upload-post/boards/pinterest - Get Pinterest boards for a profile
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const boards = await uploadPostService.getPinterestBoards(profile || undefined);

    return NextResponse.json({ success: true, boards });
  } catch (error: any) {
    console.error('Get Pinterest boards error:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to get Pinterest boards' },
      { status: 500 }
    );
  }
}
